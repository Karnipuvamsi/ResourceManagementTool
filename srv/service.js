...
module.exports = cds.service.impl(async function () {
    const { Opportunities, Customers, Projects, Employees, Demands, Allocations, newAllocations } = this.entities;
    ...
    //     console.log('✅ New Allocation Payload:', req.data);
    //     // You can return the same data or let CAP handle persistence
    //      const tx = cds.transaction(req);

    //     try {
    //         // Example: Insert using raw SQL
    //         await tx.run(`
    //     INSERT INTO Allocations (resourceID, projectID, startDate, endDate, percent)
    //     VALUES (?, ?, ?, ?, ?)
    //   `, [payload.resourceID, payload.projectID, payload.startDate, payload.endDate, payload.percent]);

    //         return payload; // return the inserted data
    //     } catch (err) {
    //         console.error('SQL Insert failed:', err);
    //         return req.error(500, 'Failed to insert allocation');
    //     }

    //     return req.data;
    // });
    // // CREATE handler for your entity
    // this.on('CREATE', newAllocations, async (req) => {
    ...
    //         // Persist
    //         const result=await INSERT.into(newAllocations).entries(entries);
    ...
    this.on('CREATE', newAllocations, async (req) => {
        ...
        try {
            ...
            if (Array.isArray(payload)) {
                const ids = entries.map(e => e.allocationId);
                const createdRows = await SELECT.from(newAllocations).where({ allocationId: { in: ids } });
                return createdRows; // return array of created entities
            } else {
                const createdRow = await SELECT.one.from(newAllocations).where({ allocationId: entries[0].allocationId });
                return createdRow; // return single created entity
            }
        } catch (err) {
            return req.error(500, 'Failed to insert allocation(s)');
        }
